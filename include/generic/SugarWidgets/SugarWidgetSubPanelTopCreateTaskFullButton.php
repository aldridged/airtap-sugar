...
class SugarWidgetSubPanelTopCreateTaskFullButton extends SugarWidget
{
    var $module;
	var $title;
	var $access_key;
...
//TODO rename defines to layout defs and make it a member variable instead of passing it multiple layers with extra copying.
	...
	*/
    ...
    function &_get_form($defines, $additionalFormFields = null, $asUrl = false)
    {
        ...
        {
            $defines['child_module_name'] = "Tasks";
        }
        else
        {
            $defines['child_module_name'] = "Tasks";
	...
	function display($defines, $additionalFormFields = null, $nonbutton = false)
	{
        ...
        if ( isset($_REQUEST['layout_def_key']) && $_REQUEST['layout_def_key'] == 'UserEAPM' ) {
            ...
            $button = "<input title='Create Task' accesskey='$this->access_key' class='button' type='submit' name='$inputID' id='$inputID' value='Create Task' onclick='javascript:document.location=\"index.php?".$megaLink."\"; return false;'/>";
        } else {
            $button = $this->_get_form($defines, $additionalFormFields);
            $button .= "<input title='Create Task' accesskey='$this->access_key' class='button' type='submit' name='$inputID' id='$inputID' value='Create Task' />\n</form>";
        }

        if ($nonbutton) {
            $button = "<a onclick=''>Create Task";
        }
        return $button;
	}
